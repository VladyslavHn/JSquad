import{b as i,l as g,r as b,a as p}from"./assets/api-5a5b1635.js";import"./assets/vendor-0cb09735.js";const u=async e=>{const t=document.querySelector(".modal-wrapper"),r=document.querySelector(".modal-icons-list"),a=document.querySelector(".modal-window");try{const o=await i.getBookDescription(e);t.innerHTML="",r.innerHTML="";const d=`
    ...
    </p>`;a.insertAdjacentHTML("beforeend",c),s.addEventListener("click",n)}else{let n=function(){g.addBookToFavorites(m),s.textContent="remove from the shopping list",s.classList.add("modal-btn-remove"),s.removeEventListener("click",n),u(o._id)};s.textContent="add to shopping list",s.addEventListener("click",n);const c=document.querySelector(".modal-text-congratulations");c!==null&&c.remove()}}catch(o){console.log("Error fetching modal:",o)}};u(bookId);const k=document.querySelector(".backdrop"),S=document.querySelector(".modal-close"),L=document.querySelector("body");S.addEventListener("click",()=>{k.classList.remove("is-open"),L.classList.remove("no-scroll")});function h(){k.classList.add("is-open"),L.classList.add("no-scroll")}h();function f(e,t){const r=document.querySelector(".bestsellers-container");r.innerHTML="",b(".bestsellers-container",t),r.insertAdjacentHTML("beforeend",'<ul class = "book-category-list"></ul>');const a=document.querySelector(".book-category-list"),o=e.map(({author:d,book_image:l,title:s,_id:m})=>p({author:d,book_image:l,title:s,_id:m})).join("");a.innerHTML=o,a.addEventListener("click",B)}async function B(e){if(e.target.nodeName==="IMG"||e.target.nodeName==="H3"||e.target.nodeName==="P"){let t=e.target.closest(".book-category-item").dataset.source;u(t),h()}}async function _(){try{const e=await i.getBestSellers();v(e)}catch(e){console.error("Error fetching best sellers:",e)}}function v(e){const t=document.querySelector(".bestsellers-container");t.innerHTML="",b(".bestsellers-container","Best Sellers Books"),t.insertAdjacentHTML("beforeend",'<ul class="bestsellers-list"></ul>');const r=document.querySelector(".bestsellers-list"),a=e.map(({books:d,list_name:l})=>`<li class="bestsellers-item">
...
`);r.insertAdjacentHTML("beforeend",a);const o=document.querySelector(".bestsellers-list");o.addEventListener("click",T),o.addEventListener("click",C)}async function C(e){if(e.preventDefault(),e.target.nodeName!=="IMG")return;let t=e.target.dataset.book;console.log(t),u(t)}async function T(e){if(e.target.nodeName!=="BUTTON")return;let t=e.target.dataset.category;const r=await i.getSelectedCategory(t);f(r,t)}const y={categoryContainer:document.querySelector(".sidebar-category-container"),categoryList:document.querySelector(".sidebar-category-list"),allCategory:document.querySelector(".all-category")};function q(e){return e.map(r=>`<li class='sidebar-category-item' data-source="${r.list_name}">${r.list_name}</li>`).join("")}(async()=>{try{const e=await i.getCategoryList(),t=q(e);y.categoryList.insertAdjacentHTML("beforeend",t)}catch(e){console.log(e)}})();y.allCategory.addEventListener("click",async e=>{try{const t=await i.getBestSellers();v(t)}catch(t){console.log(t)}});y.categoryList.addEventListener("click",async e=>{if(e.target.classList.contains("sidebar-category-item")){const t=e.target.dataset.source;document.querySelectorAll(".sidebar-category-item").forEach(a=>{a.classList.remove("category-active")}),e.target.classList.add("category-active");try{if(!e.target.classList.contains("all-category")){const a=await i.getSelectedCategory(t);f(a,t)}}catch(a){console.log(a)}}});_();
