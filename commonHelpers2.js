import{b as i,l as g,r as b,a as p}from"./assets/api-3f53b483.js";import"./assets/vendor-0cb09735.js";const u=async e=>{const o=document.querySelector(".modal-wrapper"),a=document.querySelector(".modal-icons-list"),r=document.querySelector(".modal-window");try{const t=await i.getBookDescription(e);o.innerHTML="",a.innerHTML="";const d=`
...
`);a.insertAdjacentHTML("beforeend",r);const t=document.querySelector(".bestsellers-list");t.addEventListener("click",T),t.addEventListener("click",C)}async function C(e){if(e.preventDefault(),e.target.nodeName==="IMG"||e.target.nodeName==="H3"||e.target.nodeName==="P"){let o=e.target.closest(".book-category-item").dataset.id;u(o),h()}}async function T(e){if(e.target.nodeName!=="BUTTON")return;let o=e.target.dataset.category;const a=await i.getSelectedCategory(o);f(a,o)}const y={categoryContainer:document.querySelector(".sidebar-category-container"),categoryList:document.querySelector(".sidebar-category-list"),allCategory:document.querySelector(".all-category")};function q(e){return e.map(a=>`<li class='sidebar-category-item' data-source="${a.list_name}">${a.list_name}</li>`).join("")}(async()=>{try{const e=await i.getCategoryList(),o=q(e);y.categoryList.insertAdjacentHTML("beforeend",o)}catch(e){console.log(e)}})();y.allCategory.addEventListener("click",async e=>{try{const o=await i.getBestSellers();v(o)}catch(o){console.log(o)}});y.categoryList.addEventListener("click",async e=>{if(e.target.classList.contains("sidebar-category-item")){const o=e.target.dataset.source;document.querySelectorAll(".sidebar-category-item").forEach(r=>{r.classList.remove("category-active")}),e.target.classList.add("category-active");try{if(!e.target.classList.contains("all-category")){const r=await i.getSelectedCategory(o);f(r,o)}}catch(r){console.log(r)}}});_();
